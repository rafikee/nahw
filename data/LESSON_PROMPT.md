...
- `data/course.ts` — the course manifest that defines which books exist and which lessons belong to each. You'll add the new lesson's `module_id` here.
...
- Book: <!-- e.g. book-1 (see data/course.ts for existing books) -->
...
7. Add the lesson's `module_id` to the correct book's `lessonIds` in `data/course.ts`.
